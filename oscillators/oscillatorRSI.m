function line = oscillatorRSI(firm, ~, rule, parameter)
method = rule;
lookBack = parameter;

difference = firm.candle.last - [0; firm.candle.last(1:end-1)];
g = zeros(firm.total, 1);
g(difference>0) = difference(difference>0);
l = zeros(firm.total, 1);
l(difference<0) = -difference(difference<0);
G = zeros(firm.total, 1);
L = zeros(firm.total, 1);
switch method
    case 'weighted'
...
line = struct(...
    'overSold', 0.3*ones(firm.total, 1),...
    'overBought', 0.7*ones(firm.total, 1),...
    'rsi', rsi);
end